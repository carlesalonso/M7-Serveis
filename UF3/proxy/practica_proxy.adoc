...
Configura el firefox de _PC1_ per tal que utilitzi _gollum_ com a servidor
proxy.
...
paraules: games, download o social.
...
Això ho podem fer a _saruman_ afegint les següents regles d'iptables:

----
iptables -t nat -A PREROUTING -i ethX -s ! el_teu_proxy -p tcp --dport 80 -j DNAT --to el_teu_proxy:8080
iptables -t nat -A POSTROUTING -o ethX -s xarxa_local -d el_teu_proxy -j SNAT --to iptables_box
iptables -A FORWARD -s xarxa_local -d el_teu_proxy -i ethX -o ethX -p tcp --dport 8080 -j ACCEPT
----
...
- **el_teu_proxy**: l'adreça IP del servidor on hi ha l'squid, _gollum_.
- **xarxa_local**: la xarxa local que estem configurant.
- **iptables_box**: l'adreça IP de _saruman_ que pertany a la xarxa que estem
configurant.

Amb aquestes regles aconseguim que tot el tràfic que es dirigeix al port 80
des de qualsevol màquina de la xarxa local, es desviï a _gollum_, i que la
resposta de l'squid es retorni al client a través de _saruman_, no directament.
