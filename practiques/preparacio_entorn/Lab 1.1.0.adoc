...
gràfic.
. Assignar el nom *saruman* durant la instal·lació.
...
. Basat en Debian Jessie. Màquina virtual amb 512 MB de memòria sense entorn
gràfic.
. Durant la instal·lació configurar el nom *sauron*.
. Al VirtualBox, configurar dues targetes de xarxa:
.. Adaptador pont
.. Xarxa internat intnet1
. Configuració dels paràmetres de xarxa a */etc/network/interfaces*
+
ifndef::teacher[]
.Entregar
...
+
Hi ha diverses maneres d'afegir rutes a un sistema Debian. Es poden utilitzar les
comandes *route* o *ip route*, en aquest cas les rutes *no* són permanents i
caldrà afegir-les cada cop que arranqui el sistema.
+
La segona opció és utilitzar les comandes anteriors però dins del fitxer
...
En principi les màquines virtuals sobre les que s'instal·laran servidors haurien
de ser màquines sense cap entorn gràfic instal·lat. No obstant per facilitar
la feina d'administració i per permetre capturar pantalles es permetrà la
instal·lació d'algun escriptori lleuger en aquestes màquines.
...
[source,bash]
----
root@bilbo:~# cat /etc/apt/sources.list
#
...
==== Instal·lar les VirtualBox Guest Additions en Debian
...
* També pot ser útil instal·lar alguna eina de monitoratge de xarxa, per
exemple el *WireShark* en el cas que disposeu d'entorn gràfic o el *tcpdump* en
cas contrari.
+
[source,bash]
----
...
[IMPORTANT]
====
Cal que configureu el vostre terminal per què al prompt aparegui el vostre nom
...
[source,bash]
----
Mon Jul 04 18:59:55 queraltj@bilbo:~$
----
