...
A partir d'aquí, s'afegiran mesures bàsiques de seguretat, com un filtre
antispam.
...
*sauron* tindrà una IP del rang 192.168.2.10-192.168.2.30.

Demana una IP pel teu router *sauron* i apunta-la:

- IP assignada

Configura *sauron* amb la IP assignada.

- Entrega config sauron.



== Apartat 1: Preparació de la màquina *gandalf*
...
== Apartat 2: Instal·lador del servidor de correu electrònic a _gandalf_
...
Per fer això utilitzarem el nom de domini *nomcognom.institut.lan*, on _nom_
i _cognom_ són el teu nom i el teu primer cognom.

== Apartat 3: comprovació de la configuració en local
...
== Apartat 4: Configuració del servidor DNS a la xarxa local
...
correspon al nom de domini que has utilitzat a l'apartat anterior,
*nomcognom.institut.lan* i ha de resoldre el servidor de correu a _gandalf_.
...
El Postfix té dos fitxer de configuració principals: _main.cf_, que especifica
les opcions de configuració, i _master.cf_, que especifica els serveix que
...
que no es corresponen amb els noms dels usuaris sinó, el correu dels quals
