...
`dhcp.maria.soler.local` hauria de donar la IP de _bilbo_, i
...
zone "aaa.example.com" { # Configuració de la resolució directa
    ...
    file "/etc/bind/zones/db.aaa.example.com"; # Fitxer de resolució directa
    allow-transfer { 10.128.40.72; }; # Servidor DNS secundari
};

zone "128.10.in-addr.arpa" { # Configuració de la resolució inversa
    type master;
    file "/etc/bind/zones/db.10.128";  # Resolució inversa de la subxarxa 10.128.0.0/16
